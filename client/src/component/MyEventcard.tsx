...
const MyEventCard: React.FC<EventCardProps> = ({ event, role = "user" }) => {
  ...
  const getStatusBadge = () => {
    if (role === "organizer") {
      const organizerStatus = event.adminStatus as any;
  ...
  className={`w-full min-w-[280px] max-w-lg bg-white rounded-[12px] shadow-sm border border-support1 p-5 flex flex-col h-full transition-all
    ${
      role === "organizer" && (event.adminStatus as any) === "Rejected"
        ? "" // no hover
        : "hover:shadow-md"
    }`}
>
  ...
  <div className="flex justify-between items-start w-full flex-wrap gap-2">
    <div className="flex flex-wrap items-start gap-2 sm:gap-4 flex-1">
      <h2
        className={`text-[20px] sm:text-[20px] md:text-[24px] font-bold break-words ${
          role === "organizer" && (event.adminStatus as any) === "Rejected" ? "text-support3" : "text-primary"
        }`}
      >
        {event.title}
      </h2>
    ...
    <span
      className={`text-[14px] sm:text-[16px] font-semibold px-3 sm:px-3 py-1 rounded-full whitespace-nowrap ${statusBadge.color}`}
    >
      {statusBadge.text}
    </span>
  </div>
...
{/* Buttons & Status Date */}
<div className="flex flex-col lg:flex-row justify-between items-start gap-2 mt-0 mb-0 flex-1">
  {/* Status Date (Organizer view) */}
  {role === "organizer" && (
    <div className="flex-1 flex flex-col justify-center">
      <div className="font-semibold text-text-black text-[14px] sm:text-[16px] leading-tight flex items-center gap-2">
        {(event.adminStatus as any) === "Pending" && (
          <>
            Date Submitted:{" "}
            <span className="text-support3">
              {formatStatusDate((event as any).statusDate)}
            </span>
          </>
        )}
        ...
        {(event.adminStatus as any) === "Approved" && (
          <div className="flex flex-col space-y-[2px] leading-tight">
            ...
            <span className="text-text-black">
              by {((event as any).approvedBy) || "Admin"}
            </span>
          </div>
        )}
        ...
        {(event.adminStatus as any) === "Rejected" && (
          <div className="flex flex-col space-y-[2px] leading-tight">
            ...
            <span className="text-text-black">
              by {((event as any).rejectedBy) || "Admin"}
            </span>
          </div>
        )}
      </div>
  ...
  {/* Buttons */}
  <div className="flex flex-row gap-2 w-full lg:w-auto mt-2 lg:mt-0 items-center lg:items-end">
    <button
      onClick={() => {
      ...
      className={`text-[14px] font-semibold px-3 py-2 rounded-[8px] transition w-full md:w-full lg:w-auto
        ${
          role === "organizer" && (event.adminStatus as any) === "Rejected"
    ...
    {(role === "user" && isJoined) || (role === "organizer" && (event.adminStatus as any) === "Pending") ? (
      <button
        onClick={() => setShowCancelPopup(true)}
        className="text-[14px] font-semibold bg-support1 text-text-black px-3 py-2 rounded-[8px] hover:bg-support2 transition w-full md:w-full lg:w-auto"
      >
        Cancel
      </button>
    ) : null}
