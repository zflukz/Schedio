...
const viewButtonStyles: Record<EventStatus, string> = {
  Pending: "bg-[#E5F8FC] text-[#2AA5B9] hover:bg-[#d1eef5] transition-colors",
  Approved: "bg-[#E5F8FC] text-[#2AA5B9] hover:bg-[#d1eef5] transition-colors",
  Rejected: "bg-[#F2F2F2] text-[#939393] cursor-pointer",
};

const cancelButtonStyle =
  "bg-[#F7F7F7] text-black hover:bg-[#E2E2E2] transition-colors";

const HomeOrganize: React.FC = () => {
          ...
          <div className="mt-5 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {stats.map((stat) => (
              ...
              >
                ...
                <p className={`text-[32px] font-semibold mt-2 ${stat.accent}`}>
                  {stat.value}
                </p>
              </div>
            ))}
        ...
        <section className="bg-white border border-[#ECECEC] rounded-[32px] shadow-[0px_24px_48px_rgba(16,24,40,0.06)] px-6 sm:px-10 py-10">
          <div className="flex flex-col gap-6">
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
              ...
              <Button
                className="!h-[54px] !py-[10px] !px-[24px] flex items-center justify-center gap-2 bg-[#1F1F1F] text-white text-[16px] font-semibold rounded-[18px] w-full sm:w-auto hover:!bg-[#303030]"
                onClick={() => navigate("/organizer/events/create")}
              >
                <FolderPlusIcon className="h-6 w-6" fill="white" color="black"/>
                Create Event
              </Button>
            </div>

            <div className="flex flex-col sm:flex-row gap-3 w-full items-center justify-center">
              <div className="relative max-w-[300px] sm:w-auto sm:flex-1 min-w-[220px]">
                    ...
                    {statusOptions.map((opt) => (
                      ...
                      >
                        <Checkbox
                          ...
                          onChange={(e) =>
                            handleStatusToggle(opt.name, e.target.checked)
                          }
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4">
              {filteredEvents.map((event) => (
                ...
                >
                  <div className="space-y-5">
                    <div className="flex items-center justify-between gap-3">
                      <div className="flex items-center gap-3 flex-wrap">
                        ...
                        <span
                          className={`inline-flex items-center gap-2 rounded-full px-3 py-1 font-medium text-[15px] ${
                            event.status === "Rejected"
                  ...
                  <div className="mt-6 flex items-center justify-between gap-3 flex-wrap border-t border-[#F3F3F3] pt-4">
                    ...
                    <div className="flex items-center gap-3">
                      <button
                        className={`text-[14px] font-semibold px-3 py-2 rounded-[8px] ${viewButtonStyles[event.status]}`}
                        onClick={() => {
                          navigate(`/organizer/events/${event.id}`, {
                            state: {
                              event: {
                                ...
                                totalseats: 50,
                                contactName: "Wongsakorn",
                                contactPhone: "0923397882",
                              },
                            },
                          });
                        }}
                      ...
                      {event.status === "Pending" ? (
                        <button
                          className={`text-[14px] font-semibold px-3 py-2 rounded-[8px] ${cancelButtonStyle}`}
                          onClick={() => {
            ...
            <div className="w-full max-w-[360px] rounded-[18px] bg-white p-6 text-center shadow-xl">
              ...
              <div className="mt-6 grid grid-cols-2 gap-3">
                ...
                <button
                  ...
                  onClick={() => {
                    console.log(`Confirmed cancel event ${cancelEventId}`);
